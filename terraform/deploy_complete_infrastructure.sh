...
deploy_infrastructure() {
    ...
    echo "• Kong API Gateway (1 instancia)"
    echo "• Django API (2 instancias)"
    echo "• Notifications Service (1 instancia)"
    echo "• Base de datos PostgreSQL (RDS)"
    echo "• Grupos de seguridad y networking"
    echo ""
    echo -e "${RED}COSTO ESTIMADO: ~$150-200 USD/mes${NC}"
    echo ""
...
verify_deployment() {
    ...
    # Obtener IPs/DNS para pruebas
    if terraform output kong_gateway_ip &>/dev/null; then
        KONG_IP=$(terraform output -raw kong_gateway_ip)
        info "Kong Gateway IP: $KONG_IP"
    fi
    ...
    # Probar Kong Gateway
    if [[ -n "$KONG_IP" ]]; then
        log "Probando Kong Gateway..."
        if curl -f -s "http://$KONG_IP:8000" >/dev/null; then
            log "✓ Kong Gateway está respondiendo"
        else
            warn "Kong Gateway no está respondiendo aún (esto es normal, puede tomar unos minutos más)"
        fi
    fi
